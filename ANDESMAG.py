...
# --------------------------- INTERNAL MODULES -------------------------------#
from metrics.prep_final import PreProcessor  # ADAPT (!); TR (!)
from metrics.cloud_shadow_mask import add_cloud_shadow  # ADAPT (!); TR (!)
from metrics.hill_shadow import add_hill_shadow # ADAPT (!); TR(!)
from metrics.classifier import decision_tree  # ADAPT (!); TR (!)
from metrics.main_patches_debris import extract_sla_patch  # ADAPT (!); TR(!)

#%% 
# ------------------------------- FUNCTIONS ---------------------------------#

# SPECIAL FUNCTION! NOT NESSESARY NORMALY
def extract_id_dowloaded(directory_path):
    # Lista para almacenar los últimos 16 caracteres de los nombres de las carpetas
    last_16_chars_list = []

    # Recorrer todas las carpetas en el directorio
    for root, dirs, files in os.walk(directory_path):
        for dir_name in dirs:
            # Verificar que el nombre de la carpeta tenga al menos 16 caracteres
            if len(dir_name) >= 15:
                # Extraer los últimos 16 caracteres del nombre de la carpeta
                last_16_chars = dir_name[-15:]
                # Agregarlo a la lista
                last_16_chars_list.append(last_16_chars)

        # Detener el bucle después de recorrer el nivel superior (solo las carpetas del directorio raíz)
        break
    
    return last_16_chars_list
...
def write_to_local(response, path):
    ...
    """
    # geemap.ee_export_image_collection(response, out_dir=path, scale=30,  crs='EPSG:32719')
...
# Check in users/juancruzghilardi/SIG_2018:Preparation_Module_INGV2
...
# Check in users/juancruzghilardi/SIG_2018:Cloudshadow_Mask_AC
...
# Check in 'users/juancruzghilardi/SIG_2018:Classifier_INGV2'
...
parent_facies = "C:/Users/ThinkPad/OneDrive/IANIGLA/DebrisCoverGlaciers/test_algorithm_facies"

# Define parent Directory path to save the preprocessed images (one folder per glacier)
parent_prep = "C:/Users/ThinkPad/OneDrive/IANIGLA/DebrisCoverGlaciers/test_algorithm_img"
...
# Glacier list
ing_list = ["G700681O326355S"]

batch = [(1995, 1999)] # [(start_year, end_year)]
        # # (1991, 1995),
        # # (1996, 1998),
        # # (1999, 2000),
        # # (2001, 2005),
...
while index < len(ing_list):
    for i in range(len(batch)):
        ...
        geemap.ee_export_image_collection_to_drive(map_collection, folder='CLASSIFY_' + ing_id, scale=30)
        
        # Write Preprocessed images to Google DRIVE  (USAR CUANDO LAS IMAGENES SON MUY GRANDES) 
        geemap.ee_export_image_collection_to_drive(convert_preprocessed, folder='RGB_' + ing_id , scale=30) #ADAPT (!)

        
        print("writing map")
         
        # create filename for the SLA data (csv and json)
        sla_csv_name = [str(ing_id) + "_" + str(start_year.getInfo()),'_', str(end_year.getInfo()),'_',str(doy_start.getInfo()),'_',str(doy_end.getInfo()),'_metrics.csv']
        sla_csv_name = ''.join(sla_csv_name)
        
        # agrego el ID_local del glaciar para exportar a Drive
        sla_csv_name_DRIVE = str(ing_id) + "_" + sla_csv_name[:-8]
        ...
        write_sla_dataDrive(sla_collection=SLA_data, file="TABLES_HS_" + str(ing_id), 
