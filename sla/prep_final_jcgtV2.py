...
This script composes of various helper functions but its main role is to 
prepare and provide an image collection.
...
class PreProcessor:
    ...
    Filters data from all Sensors via date and other parameters
    ...
    def __init__(
        ...
        self.THRESHOLD_CLOUD_SCORE = THRESHOLD_CLOUD_SCORE # TR (!)
        
        self.row_filter = ee.Filter.gt('WRS_PATH', 232)
        
        # Incorporo L5 (OK)
        self.l5_bands = ee.List(                                
            ["B1", "B2", "B3", "B4", "B5", "B6", "B7", "QA_PIXEL"] 
        )
        ...
        self.l7_bands = ee.List(                                
            ['B1','B2','B3','B4','B5','B6_VCID_1','B7', 'QA_PIXEL' ] 
        ...
        self.l8_bands = ee.List(
            ["B1", "B2", "B3", "B4", "B5", "B6", "B7", "B10", "B11", "QA_PIXEL"]
        ...
        self.l5_filtered = None # agregado
        self.l7_filtered = None # agregado
        self.l8_filtered = None
        self.s2_filtered = None

    def execute(self, ING): # ADAPT (!)
        """
        ...
        self.l5_filtered = self.filter_l5() # agregado
        self.l7_filtered = self.filter_l7() # agregado
        self.l8_filtered = self.filter_l8()
        self.s2_filtered = self.filter_s2()
        
      
        # Merge de todos los satelites; parece que aca hay un problema!!!
        collection = (
            self.l5_filtered.merge(self.l7_filtered).merge(self.l8_filtered).merge(self.s2_filtered)
        )
    ...
    def filter_l8(self):
        ...
        Filter L8 sensor data based on date and geometry and applies various filters
        like cloud and coverage.
        """
    ...
    def cloud_score(self, img):
        ...
        # score = score.min(self.rescale(img, "img.blue", [0.1, 0.3])) # TR (!)
        score = score.min(self.rescale(img, "img.blue", [self.THRESHOLD_CLOUD_SCORE[0], 
                                                         self.THRESHOLD_CLOUD_SCORE[1]])) # TR (!)

        # Clouds are reasonably bright in all visible bands.
        score = score.min(
            # self.rescale(img, "img.red + img.green + img.blue", [0.2, 0.8]) # TR (!)
            self.rescale(img, "img.red + img.green + img.blue", [self.THRESHOLD_CLOUD_SCORE[2],
                                                                 self.THRESHOLD_CLOUD_SCORE[3]]) # TR (!)
        )

        # Clouds are reasonably bright in all infrared bands.
        score = score.min(
            # self.rescale(img, "img.nir + img.swir1 + img.swir2", [0.3, 0.8]) # TR (!)
            self.rescale(img, "img.nir + img.swir1 + img.swir2", [self.THRESHOLD_CLOUD_SCORE[4],
                                                                  self.THRESHOLD_CLOUD_SCORE[5]]) # TR (!)

        )

        # However, clouds are not snow.
        ndsi = img.normalizedDifference(["green", "swir1"])
        # score2 = score.min(self.rescale(ndsi, "img", [0.7, 0.6])) # TR (!)
        score2 = score.min(self.rescale(ndsi, "img", [self.THRESHOLD_CLOUD_SCORE[6],
                                                      self.THRESHOLD_CLOUD_SCORE[7]])) # TR (!)
        ...
        img1_2 = img1_1.select("cloudscore").gt(self.THRESHOLD_CLOUD_SCORE[8]) # TR (!)
    ...
    def add_quality_info(self, data):
        ...
        arearatio = area_ing_outline.divide(self.area_glacier_IMG).multiply(100)

        return (
            image.set("ING_ID", self.ing_id)
            .set("areaRGIoutline", area_ing_outline)
