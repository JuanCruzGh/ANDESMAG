...
def extract_sla_patch(image):
    ...
    debrisclass = debris_vec.reduceToVectors(
        **{
            "reducer": ee.Reducer.countEvery(),  # type: ignore
            "geometry": geometry,
            "scale": 30,
    ...
    debris_vector_map = ee.FeatureCollection(debrisclass)
    ...
    # LUCAS Comment. Se pueden incluir m[as variables para ver si esta funcionando]
    feature = ee.Feature(None)
    feature = (
        feature.set("Snow Cover Ratio", snow_area_ratio)
        .set("Snow Line Altitude", snow_line)
        .set("Area", glacier_area)    
