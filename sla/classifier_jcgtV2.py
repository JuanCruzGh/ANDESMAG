...
def decision_tree(image, SRTM, ING, THRESHOLD_DECISION_TREE): # ADAPT (!); TR (!)
    ...
    score = ee.Image(1.0)
    score = score.min(
        # rescale(img=shadows, exp="img.red + img.green + img.blue", thresholds=[0.5, 2]) # TR (!)
        rescale(img=shadows, exp="img.red + img.green + img.blue", 
                thresholds=[THRESHOLD_DECISION_TREE[0], 
                            THRESHOLD_DECISION_TREE[1]]) # TR (!)
        
    )

    # snow_shadow_mask = score.select("constant").gt(0.8) # TR (!)
    snow_shadow_mask = score.select("constant").gt(THRESHOLD_DECISION_TREE[2]) # TR (!)
    
    # unknown_shadow_mask = score.select("constant").lt(0.8) # TR (!)
    unknown_shadow_mask = score.select("constant").lt(THRESHOLD_DECISION_TREE[2]) # TR (!)
    ...
    # water_mask = image_5.select("ndwi").gt(0.6) # TR (!)
    water_mask = image_5.select("ndwi").gt(THRESHOLD_DECISION_TREE[3]) # TR (!)


    # Create mask to carry on classification with everything except water
    # not_water_mask = image_5.select("ndwi").lt(0.6) # TR (!)
    not_water_mask = image_5.select("ndwi").lt(THRESHOLD_DECISION_TREE[3]) # TR (!)

    water_class = start.mask(water_mask)

    shadow = (
        # start.mask(water_class.select("blue").gt(0.1)) # TR (!)
        start.mask(water_class.select("blue").gt(THRESHOLD_DECISION_TREE[4])) # TR (!)
        .select("classification")
        .multiply(8)
    ...
    water = (
        # start.mask(water_class.select("blue").lt(0.1)) # TR (!)
        start.mask(water_class.select("blue").lt(THRESHOLD_DECISION_TREE[4])) # TR (!)
        .select("classification")
        .multiply(2)
        .unmask(-1)  # Mask everything else with minus 1 (not a class)
    )
    ...
    # snow_ice = image_5.select("ndsi").gt(0.4) # TR (!)
    snow_ice = image_5.select("ndsi").gt(THRESHOLD_DECISION_TREE[5]) # TR (!)

    # cloud_debris = image_5.select("ndsi").lt(0.4) # TR (!)
    cloud_debris = image_5.select("ndsi").lt(THRESHOLD_DECISION_TREE[5]) # TR (!)
    ...
    # low, high = nir_threshold.gt(0.41), nir_threshold.lt(0.54) # TR (!)
    low, high = nir_threshold.gt(THRESHOLD_DECISION_TREE[6]), nir_threshold.lt(THRESHOLD_DECISION_TREE[7]) # TR (!)

    is_otsu_high = low.add(high).gt(1) 

    # fixed_threshold = 0.47 # TR (!)
    fixed_threshold = THRESHOLD_DECISION_TREE[8] # TR (!)
    ...
    score = score.min(
        # rescale(ice_snow_diff, "img.red + img.green + img.blue", [0.5, 2]) # TR (!)
        rescale(ice_snow_diff, "img.red + img.green + img.blue", [THRESHOLD_DECISION_TREE[9]
                                                                  , THRESHOLD_DECISION_TREE[10]]) # TR (!)
    )

    ice = (
        # start.mask(score.select("constant").lt(0.7)) # TR (!)
        start.mask(score.select("constant").lt(THRESHOLD_DECISION_TREE[11])) # TR (!)
        .select("classification")
        .multiply(0) # antes era 0
        .unmask(1) # antes era 1
    )
    ...
    cloud = (
        # start.mask(image_7.select("red").gt(0.6)) # TR (!)
        start.mask(image_7.select("red").gt(THRESHOLD_DECISION_TREE[12])) # TR (!)
        .select("classification")
        .multiply(4) # antes era 4
        .unmask(-1)
    )

    debris = (
        # start.mask(image_7.select("red").lt(0.3)) # TR (!)
        start.mask(image_7.select("red").lt(THRESHOLD_DECISION_TREE[13])) # TR (!)
        .select("classification")
        .multiply(3)
        .unmask(-1)
    )
    ...
    classify = (
        ...
        .max(shadow_reprojected) # arriba de este estaba el hielo
        .max(snow_shadow_reprojected)
        .max(unknown_shadow_reprojected)
        .max(ice_reprojected) 
        
    ).clip(geometry_raw) # geometry
    
   
    classified = (
        classify.set("system_time_start", system_time) # classify
        .set("ING_ID", ing_id)
        .set("otsu", threshold)
        .set("deminfo", dem_info)
        #.float() # Agragado para que cambiar a float 
        #.updateMask(image.select("blue").gte(0)) #.mask(image.select("blue").gte(0)) (glacierMask.eq(1)).
        .unmask(-9999)
        .clip(geometry_raw) # geometry
    )
