...
def add_cloud_score(img, THRESHOLD_ADD_CLOUD_SCORE): # TR (!)
    ndmi = img.normalizedDifference(["nir", "swir1"])
    ndsi = img.normalizedDifference(["green", "swir1"])

    score = ee.Image(1)
    # score = score.min(rescale(img, "img.blue", [0.1, 0.5])) # TR (!)
    score = score.min(rescale(img, "img.blue", [THRESHOLD_ADD_CLOUD_SCORE[0],
                                                THRESHOLD_ADD_CLOUD_SCORE[1]])) # TR (!)

    # score = score.min(rescale(img, "img.red + img.green + img.blue", [0.2, 0.8])) # TR (!)
    score = score.min(rescale(img, "img.red + img.green + img.blue", [THRESHOLD_ADD_CLOUD_SCORE[2],
                                                                      THRESHOLD_ADD_CLOUD_SCORE[3]])) # TR (!)

    # score = score.min(rescale(ndmi, "img", [-0.1, 0.1])) # TR (!)
    score = score.min(rescale(ndmi, "img", [THRESHOLD_ADD_CLOUD_SCORE[4],
                                            THRESHOLD_ADD_CLOUD_SCORE[5]])) # TR (!)
    
    # score = score.min(rescale(ndsi, "img", [0.4, 0.1])) # TR (!)
    score = score.min(rescale(ndsi, "img", [THRESHOLD_ADD_CLOUD_SCORE[6],
                                            THRESHOLD_ADD_CLOUD_SCORE[7]])) # TR (!)
...
def project_shadows(
   ...
   # image = image.addBands(ee.Image.constant(0).rename(["cloudShadow"]))
    return image


def add_cloud_shadow(image, ING, THRESHOLD_ADD_CLOUD_SHADOW, THRESHOLD_ADD_CLOUD_SCORE): # ADAPT (!); TR (!)
    id_glacier  = image.get("ING_ID")
    # cloud_threshold = 20 # TR (!)
    cloud_threshold = THRESHOLD_ADD_CLOUD_SHADOW[0] # TR (!)
    cloud_heights = ee.List.sequence(0, 500, 25)
    contract_pixels = 10
    dilate_pixels = 15
    # infrared_threshold = 0.45 # TR (!)
    infrared_threshold = THRESHOLD_ADD_CLOUD_SHADOW[1] # TR (!)
    geometry = ING.filterMetadata("rgi_id", "equals", id_glacier ) # ADAPT (!)

    # Get cloud score
    image = add_cloud_score(image, THRESHOLD_ADD_CLOUD_SCORE) # TR (!)
